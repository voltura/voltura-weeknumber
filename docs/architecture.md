...
`App` owns single-instance activation and awaited shutdown. `AppRuntime` composes services and routes application actions. `MainWindow` owns navigation, activation, layout, and the global always-on-top state. The header pin and the Preferences checkbox change one persisted setting immediately while preserving unrelated draft edits; WPF owns the resulting topmost behavior without polling or focus enforcement. `CalendarViewModel` presents the selected date, moves it by whole weeks, and resolves signed week offsets from the current local date; `WeekLookupViewModel` resolves week numbers to date ranges, `DateSpanViewModel` compares two dates, and `DateLookupViewModel` handles day-of-year and Julian-day conversions. Date views follow today until a date is selected or navigated; Today and a zero-week offset resume automatic date changes. `SettingsEditor` keeps preference edits in a draft until Save. The automatic-update toggle on About saves immediately while preserving other draft edits. Stable choice objects preserve selections when the display language changes.

## Calendar and tray

The Calendar tab uses `CalendarBrowserViewModel` for transient Year → Month → Week navigation. It starts at the current Gregorian year and retains its view and date during the application session. Month cards list whole weeks and their date ranges; month rows open seven-day views. Breadcrumbs return to the containing month or year, arrows move by the displayed period, and Today recenters that view and resumes following the local date. Browsing Calendar does not change the separate Week number lookup.

`CalendarPeriod` builds complete week rows using the same convention resolution and calculations as `WeekCalculator`. Adjacent months share boundary weeks. If a regional week changes number within one row, both numbers are shown. Unsupported regional periods show an unavailable message for the entire view; dates beyond years 1–9999 remain blank weekday slots. Formatting follows the display language independently of the calculation culture. The existing runtime refresh updates today's highlight; calendar projections are only built while the tab is active. No additional timer or persisted settings are introduced.
...
The Date span tab compares either date order using the half-open interval from the earlier date up to, but not including, the later date. Monday through Friday are weekdays; Saturday and Sunday are weekend days. Whole weeks are complete seven-day periods. Each endpoint follows the local date independently until edited, and its Today action resumes that behavior. Results and selections are transient, require no network or holiday data, and are not persisted.
