...
public partial class DateLookupPage : UserControl
{
    ...
    private bool ValidInsertion(TextBox input, string text) =>
        ValidInput(
            input,
            input
    ...
    private void NumberTextInput(object sender, TextCompositionEventArgs args) =>
        args.Handled = !ValidInsertion((TextBox)sender, args.Text);
    ...
    private void NumberPasting(object sender, DataObjectPastingEventArgs args)
    {
        if (
            ...
            || !ValidInsertion((TextBox)sender, text)
        )
        {
            args.CancelCommand();
        }
    ...
    {
        if (
            DataContext is DateLookupViewModel { ShowYear: true }
            && sender is TextBox { Text.Length: 0 } input
        )
        {
            input.SetCurrentValue(
                TextBox.TextProperty,
    ...
    private void TodayClick(object sender, RoutedEventArgs args) =>
        ((DateLookupViewModel)DataContext).Today();

    private void ConvertClick(object sender, RoutedEventArgs args) =>
        ((DateLookupViewModel)DataContext).Convert();
}
