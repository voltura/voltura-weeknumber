...
public sealed class SettingsStore(string directory) : IDisposable
{
    ...
    {
        if (File.Exists(FilePath)) Current = await ReadAsync(FilePath);
    }

    public static async Task<AppSettings> ReadAsync(string path)
    {
        await using var stream = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, true);
        if (stream.Length > MaximumBytes) throw new InvalidDataException("Settings file is too large.");
        ...
        if (count > MaximumBytes) throw new InvalidDataException("Settings file is too large.");
        var settings = JsonSerializer.Deserialize<AppSettings>(bytes.AsSpan(0, count), JsonOptions)
