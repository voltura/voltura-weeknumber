...
{
    public static IconAppearance Resolve(AppSettings settings, bool darkTaskbar, bool highContrast = false)
    {
        if (highContrast) return new(SystemColors.WindowTextColor, SystemColors.WindowColor);
        if (!settings.AutomaticIcon) return new(Parse(settings.Foreground), Parse(settings.Background));
        return darkTaskbar ? new(Parse("#FFF2F5FA"), Parse("#FF202936")) : new(Parse("#FF233047"), Parse("#FFF9FBFF"));
    }
...
{
    public static IReadOnlyList<int> Sizes { get; } = Array.AsReadOnly(new[] { 16, 20, 24, 28, 32, 36, 40, 48, 64, 96, 128, 256 });

    public static BitmapSource Render(int week, int size, IconAppearance appearance)
    {
        if (week is < 1 or > 54) throw new ArgumentOutOfRangeException(nameof(week));
        ...
        using (var dc = visual.RenderOpen())
        {
            ...
            dc.DrawRoundedRectangle(bg, pen, new Rect(inset, top + inset, size - stroke, size - top - stroke), size < 24 ? 1 : size * .08, size < 24 ? 1 : size * .08);
            var header = Math.Round(size * .29);
            dc.DrawLine(pen, new Point(stroke, header + inset), new Point(size - stroke, header + inset));
            foreach (var x in new[] { Math.Round(size * .28), Math.Round(size * .72) })
                dc.DrawRoundedRectangle(fg, null, new Rect(x - stroke / 2, 0, stroke, Math.Max(3, size * .21)), stroke / 2, stroke / 2);
            var fontSize = size * (size <= 24 ? .69 : .65);
            var text = new FormattedText(week.ToString("D2", CultureInfo.InvariantCulture), CultureInfo.InvariantCulture,
                FlowDirection.LeftToRight, new Typeface(new FontFamily("Segoe UI"), FontStyles.Normal, FontWeights.Bold, FontStretches.Condensed), fontSize, fg, 1);
    ...
    public static byte[] EncodeIco(int week, IconAppearance appearance, IReadOnlyList<int>? sizes = null)
    {
        sizes ??= Sizes;
    ...
    public static void CreateReviewSheet(string outputPath)
    {
        ...
        using (var dc = visual.RenderOpen())
        {
            ...
            {
                var y = dark ? 530 : 0;
                dc.DrawRectangle(dark ? new SolidColorBrush(Color.FromRgb(16, 20, 28)) : Brushes.White, null, new Rect(0, y, 1100, 530));
                var appearance = IconAppearance.Resolve(new(), dark);
                dc.DrawText(new FormattedText(dark ? "Dark taskbar • 01–53" : "Light taskbar • 01–53", CultureInfo.InvariantCulture,
                    FlowDirection.LeftToRight, new Typeface("Segoe UI"), 22, dark ? Brushes.White : Brushes.Black, 1), new Point(20, y + 12));
                for (var week = 1; week <= 53; week++)
                {
                    var col = (week - 1) % 18; var row = (week - 1) / 18;
                    dc.DrawImage(Render(week, 16, appearance), new Rect(22 + col * 60, y + 60 + row * 52, 16, 16));
                    dc.DrawImage(Render(week, 24, appearance), new Rect(42 + col * 60, y + 56 + row * 52, 24, 24));
                }
                var x = 20;
