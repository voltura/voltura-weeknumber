...
internal sealed partial class NativeTray : IDisposable
{
    ...
    internal bool IsPerMonitorV2 => AreDpiAwarenessContextsEqual(GetWindowDpiAwarenessContext(_source.Handle), new nint(-4));
    ...
    public void Update(int week, IconAppearance appearance, string tooltip)
    {
        ...
        if (_rendered != key)
        {
            ...
            var handle = CreateIconFromResourceEx(png, (uint)png.Length, true, 0x30000, size, size, 0);
            if (handle == 0) throw new Win32Exception(Marshal.GetLastWin32Error());
            var replacement = new SafeIconHandle(handle);
            var old = _icon;
            _icon = replacement;
            _rendered = key;
            try { Publish(tooltip, true); }
            finally { old?.Dispose(); }
        }
        else if (_tooltip != tooltip || !_added) Publish(tooltip, false);
    }
    ...
    private NotifyIconData Data() => new()
    {
        ...
        CallbackMessage = Callback, Icon = _icon?.DangerousGetHandle() ?? 0,
        Tip = _tooltip, Info = string.Empty, InfoTitle = string.Empty
    };

    private int IconSize()
    {
        var identity = new NotifyIconIdentifier { Size = (uint)Marshal.SizeOf<NotifyIconIdentifier>(), Window = _source.Handle, Id = 1 };
    ...
    {
        if ((uint)message == _taskbarCreated) { _added = false; _rendered = null; DisplayChanged?.Invoke(); }
        else if (message is 0x7E or 0x2E0 or 0x1A) DisplayChanged?.Invoke();
        else if (message == Callback)
        {
            var code = (int)(lParam.ToInt64() & 0xffff);
            if (code is 0x203 or 0x401) OpenRequested?.Invoke();
            else if (code == 0x405) NotificationClicked?.Invoke();
            else if (code == 0x7B)
            {
                RebuildMenu();
                _ = SetForegroundWindow(_source.Handle);
                var packed = wParam.ToInt64();
                _menu.Show(new System.Drawing.Point((short)(packed & 0xffff), (short)((packed >> 16) & 0xffff)));
            }
        }
        return 0;
    }
    ...
    [LibraryImport("shell32.dll", EntryPoint = "Shell_NotifyIconGetRect")] private static partial int ShellNotifyIconGetRect(ref NotifyIconIdentifier id, out NativeRect rectangle);
    [LibraryImport("user32.dll", EntryPoint = "RegisterWindowMessageW", StringMarshalling = StringMarshalling.Utf16)] private static partial uint RegisterWindowMessage(string message);
    ...
    [LibraryImport("user32.dll")] private static partial nint MonitorFromRect(in NativeRect rectangle, uint flags);
...
{
    internal SafeIconHandle(nint handle) : base(true) => SetHandle(handle);
