...
internal sealed class SingleInstance : IDisposable
{
    ...
    public SingleInstance(string dataDirectory)
    {
        ...
        IsFirst = created;
        _activation = new EventWaitHandle(false, EventResetMode.AutoReset, @"Local\VolturaWeekNumber-Activate-" + identity);
        if (!created) _activation.Set();
    }
    public void Listen(Action activate) => _wait = ThreadPool.RegisterWaitForSingleObject(_activation, (_, _) => activate(), null, Timeout.Infinite, false);
    public void Dispose()
    {
