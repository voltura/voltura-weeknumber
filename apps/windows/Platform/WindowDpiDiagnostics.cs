...
internal static partial class WindowDpiDiagnostics
{
    ...
    internal static object Capture(Window window)
    {
        ...
        return new
        {
            ...
            nativeDpi = GetDpiForWindow(handle),
            perMonitorV2 = AreDpiAwarenessContextsEqual(
                GetWindowDpiAwarenessContext(handle),
                new nint(-4)
            ),
            monitor = monitor.ToInt64(),
            monitorDpiX = monitorX,
            monitorDpiY = monitorY,
            wpfDpiX = dpi.PixelsPerInchX,
            wpfDpiY = dpi.PixelsPerInchY,
            left = rect.Left,
            top = rect.Top,
            pixelWidth = rect.Right - rect.Left,
            pixelHeight = rect.Bottom - rect.Top,
            logicalWidth = window.ActualWidth,
