...
internal sealed class AppRuntime : IAsyncDisposable
{
    ...
    public async Task StartAsync(bool hidden)
    {
        ...
        if (!hidden || _settingsDamaged)
        {
            Window.Open();
        }

        if (_settings.Current.StartupNotification && !_paths.Isolated)
        {
            _tray.Notify(
                "Voltura WeekNumber",
                Strings.Current["Started"],
                _settings.Current.SilentNotifications
            );
        }
    ...
    internal Task ApplyReviewSettingsAsync(AppSettings value)
    {
        if (!_paths.Isolated)
        {
            throw new InvalidOperationException("Review settings require an isolated profile.");
        }
    ...
    private void OnPreferencesChanged(object sender, UserPreferenceChangedEventArgs args) =>
        QueueRefresh();

    private void OnPowerChanged(object sender, PowerModeChangedEventArgs args)
    {
        ...
        _ = Window.Dispatcher.BeginInvoke(
            () =>
            {
                ...
                if (_traceDpi)
                {
                    _log.Record(
                        "DPI system-refresh "
                            + JsonSerializer.Serialize(WindowDpiDiagnostics.Capture(Window))
                    );
                }
            },
    ...
    internal void Refresh(bool notify, DateTimeOffset now)
    {
        ...
        var firstDay =
            settings.Calendar.Mode == CalendarMode.Regional
                ? region.DateTimeFormat.FirstDayOfWeek
                : settings.Calendar.FirstDay;
        var rule =
            settings.Calendar.Mode == CalendarMode.Regional
    ...
    private void UpdateChanged()
    {
        ...
        _ = Window.Dispatcher.BeginInvoke(() =>
        {
            ...
            Window.UpdateState(state, _updates.Eligible);

            if (state.Ready && !_updateReadyAnnounced && !_paths.Isolated)
            {
    ...
    private async Task ExecuteAsync(string action)
    {
        ...
        {
            case "save":
                await SaveAsync(Model.Editor.Value);
                break;
            case "auto-updates":
            ...
            case "background":
                ...
                if (colorPicker.ShowDialog() == true)
                {
                    if (foreground)
                    {
                        Model.Editor.Foreground = colorPicker.SelectedColor;
                    }
                    else
                    {
                        Model.Editor.Background = colorPicker.SelectedColor;
                    }
                }
            ...
            case "import":
                ...
                {
                    await SaveAsync(await SettingsStore.ReadAsync(open.FileName), true);
                }
            ...
            case "export":
                ...
                {
                    await SettingsStore.WriteAsync(export.FileName, _settings.Current);
            ...
            case "export-icon":
                ...
                {
                    var settings = _settings.Current;
                    var week = WeekCalculator.Calculate(
            ...
            case "log":
                ...
                {
                    await File.WriteAllTextAsync(_log.FilePath, string.Empty);
                }
