...
public sealed class CalendarTests
{
    ...
    {
        var result = WeekCalculator.Calculate(new(year, month, day), new(CalendarMode.Iso), CultureInfo.GetCultureInfo("en-US"));
        Assert.Equal(number, result.Number); Assert.Equal(isoYear, result.IsoYear);
    }
    [Theory]
    [InlineData("sv-SE")]
    [InlineData("de-DE")]
    ...
    {
        var result = WeekCalculator.Calculate(new(2024, 12, 31), new(), CultureInfo.GetCultureInfo(region));
        Assert.Equal(1, result.Number); Assert.Equal(2025, result.IsoYear);
    }
    [Fact]
    public void EveryCustomRuleUsesItsOwnConvention()
    {
        ...
        foreach (var date in new[] { new DateTime(2024, 12, 31), new DateTime(2021, 1, 1), new DateTime(2024, 2, 29) })
        {
            var result = WeekCalculator.Calculate(DateOnly.FromDateTime(date), new(CalendarMode.Custom, first, rule), region);
            var expected = first == DayOfWeek.Monday && rule == CalendarWeekRule.FirstFourDayWeek ? ISOWeek.GetWeekOfYear(date) : region.Calendar.GetWeekOfYear(date, rule, first);
