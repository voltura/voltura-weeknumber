...
public sealed class RuntimePerformanceTests(WpfTestFixture fixture)
{
    ...
    public void UnchangedThemeReusesFluentResourcesAndBrushes()
    {
        fixture.Run(() =>
        {
            ...
            string[] keys = ["WindowBrush", "SurfaceBrush", "TextBrush", "MutedBrush", "AccentBrush", "BorderBrush"];
            try
            {
...
#pragma warning disable WPF0001
                    Assert.Equal(choice switch { "light" => ThemeMode.Light, "dark" => ThemeMode.Dark, _ => ThemeMode.System }, app.ThemeMode);
#pragma warning restore WPF0001
                    var dictionaries = app.Resources.MergedDictionaries.ToArray();
                    var brushes = keys.Select(key => app.Resources[key]).ToArray();
                    for (var i = 0; i < 100; i++) ThemeManager.Apply(choice);
                    Assert.Equal(dictionaries.Length, app.Resources.MergedDictionaries.Count);
                    for (var i = 0; i < dictionaries.Length; i++) Assert.Same(dictionaries[i], app.Resources.MergedDictionaries[i]);
                    for (var i = 0; i < keys.Length; i++) Assert.Same(brushes[i], app.Resources[keys[i]]);
                }
            }
            finally { ThemeManager.Apply("system"); }
        });
    }
    ...
    public void HighContrastTransitionReloadsFluentEvenWhenThemeChoiceIsUnchanged()
    {
        fixture.Run(() =>
        {
            ...
            typeof(ThemeManager).GetField("_highContrast", BindingFlags.Static | BindingFlags.NonPublic)!
                .SetValue(null, !SystemParameters.HighContrast);
            ThemeManager.Apply("system");
            Assert.Contains(dictionaries, dictionary => !resources.MergedDictionaries.Contains(dictionary));
            var updated = resources.MergedDictionaries.ToArray();
            ThemeManager.Apply("system");
            for (var i = 0; i < updated.Length; i++) Assert.Same(updated[i], resources.MergedDictionaries[i]);
        });
    }
        ...
        fixture.Run(() =>
        {
            ...
            try
            {
                foreach (var dark in PaletteModes)
                {
                    ...
                    var normal = keys.Select(key => Assert.IsType<SolidColorBrush>(resources[key]).Color).ToArray();
                    Assert.Equal((Color)ColorConverter.ConvertFromString(dark ? "#111720" : "#F4F6FA"), normal[0]);
                    Assert.Equal((Color)ColorConverter.ConvertFromString(dark ? "#F1F5FB" : "#17253B"), normal[2]);
                    ThemeManager.ApplyPalette(dark, true);
                    Color[] expected = [SystemColors.WindowColor, SystemColors.WindowColor, SystemColors.WindowTextColor,
                        SystemColors.WindowTextColor, SystemColors.HighlightColor, SystemColors.WindowTextColor];
                    for (var i = 0; i < keys.Length; i++)
                    {
                        Assert.Equal(expected[i], Assert.IsType<SolidColorBrush>(resources[keys[i]]).Color);
                    ...
                    ThemeManager.ApplyPalette(dark, false);
                    for (var i = 0; i < keys.Length; i++) Assert.Equal(normal[i], Assert.IsType<SolidColorBrush>(resources[keys[i]]).Color);
                }
            }
            finally { ThemeManager.Apply("system"); }
        });
    ...
    public void PreviewReusesOnlyMatchingWeekAndColorsWhileDateAndLanguageStayCurrent()
    {
        fixture.Run(() =>
        {
            ...
            try
            {
                var settings = new AppSettings { Calendar = new() { Mode = CalendarMode.Iso }, AutomaticIcon = false };
                ...
                for (var i = 0; i < 100; i++) model.Refresh();
                ...
                foreach (var changed in new[] { settings with { Background = "#FF123456" }, settings with { Foreground = "#FF987654" } })
                {
                    preview = model.Preview;
                    var oldAppearance = IconAppearance.Resolve(settings, false, SystemParameters.HighContrast);
                    var newAppearance = IconAppearance.Resolve(changed, false, SystemParameters.HighContrast);
                    model.Apply(changed);
                    model.Refresh();
                    if (oldAppearance == newAppearance) Assert.Same(preview, model.Preview);
                    else Assert.NotSame(preview, model.Preview);
                    settings = changed;
                }
            ...
            finally { Strings.Current.SetLanguage("en"); }
        });
    }
    ...
    public async Task StartupAndSettingsSaveRefreshCalendarOnceEach()
    {
        ...
        try
        {
            fixture.Run(() =>
            {
                ...
                runtime.Model.PropertyChanged += (_, args) => { if (args.PropertyName is null) refreshes++; };
            ...
            fixture.Run(() =>
            {
                ...
                operation = runtime.ApplyReviewSettingsAsync(new AppSettings { Language = "de", AutomaticIcon = false, Background = "#FF123456" });
            });
            await operation;
            fixture.Run(() =>
            {
    ...
    public void TrayMenuReusesItemsAndUpdatesTranslatedCommands()
    {
        fixture.Run(() =>
        {
            ...
            try
            {
                var menu = Assert.IsType<System.Windows.Forms.ContextMenuStrip>(typeof(NativeTray)
                    .GetField("_menu", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(tray));
                var firstItem = menu.Items[0];
                var renderer = menu.Renderer;
                for (var index = 0; index < 100; index++) tray.RebuildMenu();
