...
if (-not (Test-Path -LiteralPath $publicPath)) { throw 'Configure the dedicated production public key before building release artifacts.' }
$passphrase = Read-Host 'Update signing key passphrase' -AsSecureString
$credential = [Net.NetworkCredential]::new('', $passphrase)
$rsa = [Security.Cryptography.RSA]::Create()
try {
    $rsa.ImportFromEncryptedPem([IO.File]::ReadAllText([IO.Path]::GetFullPath($KeyPath)), $credential.Password)
    if ($rsa.ExportSubjectPublicKeyInfoPem().Trim() -ne [IO.File]::ReadAllText($publicPath).Trim()) { throw 'Signing key does not match the embedded public key.' }
    ...
    $assets = @('','-full') | ForEach-Object {
        ...
        [ordered]@{name=$name;size=$file.Length;sha256=(Get-FileHash -LiteralPath $file.FullName -Algorithm SHA256).Hash.ToLowerInvariant()}
    }
    $bytes = [Text.Encoding]::UTF8.GetBytes((ConvertTo-Json -InputObject ([ordered]@{schema=1;version=$version;assets=@($assets)}) -Depth 5 -Compress))
...
} finally { $rsa.Dispose(); $passphrase.Dispose(); $credential = $null }
